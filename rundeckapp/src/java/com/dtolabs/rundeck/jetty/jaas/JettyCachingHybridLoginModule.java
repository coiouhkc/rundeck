 ...
 */
public class JettyCachingHybridLoginModule extends AbstractLoginModule {
...
//        Log.debug("Searching for users with filter: \'" + filter + "\'" + " from base dn: " + _userBaseDn);
    ...
    private List getUserRoles(DirContext dirContext, String username) throws LoginException,
        ...
        //return getUserRolesByDn(dirContext, userDn, username);
				return propfileModule.getUserInfo(username).getRoleNames();
    }
    ...
    private List getUserRolesByDn(DirContext dirContext, String userDn, String username) throws LoginException,
        ...
        if (dirContext == null || _roleBaseDn == null || (_roleMemberAttribute == null
            ...
            Log.warn("JettyCachingHybridLoginModule: No user roles found: roleBaseDn, roleObjectClass and roleMemberAttribute or roleUsernameMemberAttribute must be specified.");
            return roleList;
        }
        ...
        if (roleList.size() < 1) {
            Log.warn("JettyCachingHybridLoginModule: User '" + username + "' has no role membership; role query configuration may be incorrect");
        }else{
            Log.debug("JettyCachingHybridLoginModule: User '" + username + "' has roles: " + roleList);
        }
    ...
    protected boolean bindingLogin(String username, Object password) throws LoginException,
        ...
        environment.put(Context.SECURITY_CREDENTIALS, (pass.trim().isEmpty() ? "password_that_would_never_be_used" : pass));
        ...
        //List roles = getUserRolesByDn(dirContext, userDn, username);
				List roles = propfileModule.getUserInfo(username).getRoleNames();
